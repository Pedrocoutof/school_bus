<script setup>
import { ref, onMounted } from "vue";
import axios from "axios"; // Certifique-se de importar o axios

const theads = ref([]);
const data = ref([]);
const hasData = ref(false);

const props = defineProps({
    url: {
        type: String,
        required: true
    },
});

function teste(id) {
    console.log(id);
}

onMounted(async () => {
    try {
        const response = await axios.get(props.url);
        hasData.value = response.data.length > 0;

        if (hasData.value) {
            theads.value = Object.keys(response.data[0]);
            data.value = response.data;
        }
    } catch (error) {
        console.error(error);
    }
});
</script>

<template>
    <table v-if="hasData" class="table-auto w-full">
        <thead>
        <tr>
            <th
                v-for="(thead, index) in theads"
                :key="thead"
                :class="{
                        'rounded-tl-lg': index === 0,
                        'rounded-tr-lg': index === theads.length - 1,
                        'hidden': thead === 'id'
            }"
            class="text-gray-600 py-3 bg-gray-100 text-start pl-2"
            >
            {{ thead }}
            </th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="(row, rowIndex) in data" :key="rowIndex" @click="teste(data[rowIndex].id)" class="hover:bg-gray-50">
            <td v-for="(value, key) in row" :key="key" :class="{'hidden': key === 'id'}" class="border-b border-slate-100 py-4 pl-2">
                {{ value }}
            </td>
        </tr>
        </tbody>
    </table>
    <div v-else-if="!hasData" class="flex items-center justify-center">
        <svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 -960 960 960" width="48px" fill="#D9D9D9">
            <path
                d="M484-247q16 0 27-11t11-27q0-16-11-27t-27-11q-16 0-27 11t-11 27q0 16 11 27t27 11Zm-35-146h59q0-26 6.5-47.5T555-490q31-26 44-51t13-55q0-53-34.5-85T486-713q-49 0-86.5 24.5T345-621l53 20q11-28 33-43.5t52-15.5q34 0 55 18.5t21 47.5q0 22-13 41.5T508-512q-30 26-44.5 51.5T449-393Zm31 313q-82 0-155-31.5t-127.5-86Q143-252 111.5-325T80-480q0-83 31.5-156t86-127Q252-817 325-848.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 82-31.5 155T763-197.5q-54 54.5-127 86T480-80Zm0-60q142 0 241-99.5T820-480q0-142-99-241t-241-99q-141 0-240.5 99T140-480q0 141 99.5 240.5T480-140Zm0-340Z"
            />
        </svg>
        <h3 class="pl-4">Nenhum dado encontrado</h3>
    </div>
</template>

<style scoped>
.hidden {
    display: none;
}
</style>
